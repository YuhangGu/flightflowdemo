/**
 * Created by Aero on 17/03/2017.
 */

var graphicFlight2D = {
    map_width : $("#flow-map").width(),
    map_height : 700,
    projection : null,
    path : null,
    svg : null
...
function initFlightFlowMap(){

    graphicFlight2D.projection = d3.geo.azimuthalEquidistant()
        .scale(150)
        .translate([graphicFlight2D.map_width / 2, graphicFlight2D.map_height / 2])
        .clipAngle(180 - 1e-3)
        .precision(.1);

    //Define path generator
    graphicFlight2D.path = d3.geo.path().projection(graphicFlight2D.projection);

    graphicFlight2D.svg = d3.select("#flow-map").append("svg")
        .attr("id","svg_flow")
        .attr("width", graphicFlight2D.map_width)
        .attr("height", graphicFlight2D.map_height)
        .attr("transform", "rotate(0,180,180)")
        .attr("transform", "translate(" + (-30)+"," + 50 + ")");

    var g_basemap = graphicFlight2D.svg.append("g")
        .attr("class","basemap");
    ...
    var g_arcflows = graphicFlight2D.svg.append("g").attr("id","group_flow");
    ...
    function drawAirLines(  ) {
        ...
        d3.json("../data/arrivalflows.json", function (flights) {
            ...
            g_arcflows.selectAll("arc")
                ...
                .attr("d", function(d){
                    ...
                    var p0 = graphicFlight2D.projection(d.geometry.coordinates[0]),
                        p2 = graphicFlight2D.projection(d.geometry.coordinates[1]);
        ...
        graphicFlight2D.svg.append("rect")
            .attr("class", "overlay")
            .attr("width", graphicFlight2D.map_width)
            .attr("height", graphicFlight2D.map_height)
            .call(zoom);
    ...
    d3.select(self.frameElement).style("height", graphicFlight2D.map_height + "px");
